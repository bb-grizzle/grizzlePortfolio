...
function showLinkBtn(el){	
	...
	linkBtn.setAttribute('href', `/project/${hrefLink}`);
...
function setDetailImage_y(){
	...
	for(i of listMove){
		console.log(i);
		i.style.transform = 'translateY(' + (text_h * 2) + 'px)';
	}
}
...
//set data
		arr_titles = projects.map(project=>{
			return project.title;
		});
		arr_descript_s = projects.map(project=>{
			return project.descript_s;
		});
		arr_descript_l = projects.map(project=>{
			return project.descript_l;
		});
		arr_tag = projects.map(project=>{
			return project.tag;
		});
		arr_thum_color = projects.map(project=>{
			return project.thum_color;
		});
		arr_thum_bw = projects.map(project=>{
			return project.thum_bw;
		});
		arr_detail = projects.map(project=>{
			return project.detail;
		});
		arr_linkStatus = projects.map(project=>{
			return project.linkStatus;
		});
		arr_link = projects.map(project=>{
			return project.link;
		});
	})
	.then(function(){
